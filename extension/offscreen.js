// Audio playback for ClarkReader.
//
...
class Player {
  ...
  async playFrom(i, token) {
    ...
    try {
      ...
      this.teardown();
      report({ type: "cr-playback-error", message: err.message });
      return;
    }
  ...
  stop({ silent = false } = {}) {
    ...
    if (had && !silent) report({ type: "cr-stopped" });
  }
  ...
  async toggle() {
    ...
    report({
      ...
      index: this.index,
      total: this.count,
      text: this.job.chunks[this.index]?.text ?? "",
      state: this.ctx.state === "running" ? "playing" : "paused",
    });
  }
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === "control") {
    if (msg.action === "toggle") player.toggle();
    else if (msg.action === "stop") player.stop();
    else if (msg.action === "next") player.skip(1);
    else if (msg.action === "prev") player.skip(-1);
  }
});
